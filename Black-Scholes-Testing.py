...
def option_pricing_and_var(ticker, option_strike, current_stock_price, time_to_expiration, risk_free_rate):
    ...
    # Calculate VaR for stock itself
    confidence_levels = [0.95, 0.99, 0.975]
    var_dict_underlying = calculate_var(daily_returns, confidence_levels=confidence_levels)
    ...
    plt.xticks(index, list(var_dict_underlying.keys()))
    plt.legend()
    plt.show()


# Unit Tests for the Black-Scholes model and other functions
class TestOptionPricing(unittest.TestCase):
    ...
    def test_black_scholes_call_price(self):
        ...
        expected_price = black_scholes(S, X, T, r, sigma, 'call')
        self.assertAlmostEqual(expected_price, 10.4506, places=4,
                               msg="Call option price does not match the expected value.")
    ...
    def test_black_scholes_put_price(self):
        ...
        """
        S, X, T, r, sigma = 100, 100, 1, 0.05, 0.2
        expected_price = black_scholes(S, X, T, r, sigma, 'put')
        self.assertAlmostEqual(expected_price, 5.5735, places=4,
                               msg="Put option price does not match the expected value.")

    def test_black_scholes_invalid_option_type(self):
        """
        Test for invalid option type handling.
        """
        S, X, T, r, sigma = 100, 100, 1, 0.05, 0.2
        with self.assertRaises(ValueError, msg="Invalid option type should raise a ValueError."):
            black_scholes(S, X, T, r, sigma, 'invalid_type')

    def test_calculate_volatility(self):
        """
        Test the calculation of annualized historical volatility.
        """
        # Use a mock ticker
        ticker = 'AAPL'
        # Mock the historical data
        mock_hist = pd.DataFrame({
            'Close': [100, 102, 101, 105, 107, 110, 108, 107, 106, 108]
        }, index=pd.date_range(start='2024-01-01', periods=10))

        with patch.object(yf.Ticker, 'history', return_value=mock_hist):
            sigma, daily_returns, hist = calculate_volatility(ticker, period='1y')
            self.assertIsInstance(sigma, float)
            self.assertGreater(sigma, 0)

    def test_calculate_var(self):
        """
        Test the Value at Risk (VaR) calculation.
        """
        # Mock returns data
        returns = pd.Series([0.01, -0.02, 0.015, -0.01, 0.02])
        confidence_levels = [0.95, 0.99, 0.975]
        var_dict = calculate_var(returns, confidence_levels=confidence_levels)

        # Check if the dictionary contains all expected keys
        expected_keys = [f'{cl * 100:.0f}%' for cl in confidence_levels]
        for key in expected_keys:
            self.assertIn(key, var_dict, msg=f"Key {key} not found in VaR dictionary.")

        # Check that all VaR values are positive
        for key in var_dict:
            self.assertGreater(var_dict[key], 0, msg=f"VaR value for {key} should be greater than 0.")
